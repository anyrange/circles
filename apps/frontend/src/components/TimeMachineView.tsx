...
import { useTimeMachine } from "@/lib/use-time-machine";
      ...
      {data &&
        ...
        data.map((yearEntry) => (
          <Card key={yearEntry.year}>
            <CardHeader>
              <CardTitle className="text-base">{yearEntry.year}</CardTitle>
            </CardHeader>
            <CardContent>
                ...
                {yearEntry.plays.slice(0, 20).map((play, i) => (
                  <li key={i}>
                    <TrackRow
                      ...
                      to={`/tracks/${play.track.id}`}
                      leading={new Date(play.playedAt).toLocaleTimeString([], {
                        hour: "2-digit",
                        minute: "2-digit",
                        hour12: false,
                      })}
